...
*  Lab files are already downloaded into user's working directory: **apicem-ga-1.2-ll-sample-code-basic-labs**
...
1. Open a terminal and switch to the directory that holds the learning lab sample code.  Typically, the**`apicem-ga-1.2-ll-sample-code-basic-labs`**is your working directory.
2
...
## Sample scripts
All Python sample scripts are working in a flat directory. **apicem\_config.py** is used for configuring parameters in different environment. IThose parameters are APIC-EM IP (including port number), username, password and the apic-em API version number. If you are using your own APIC-EM this is the file you need to modify - **apicem.py**. This script contains common functions. Every script needs to import apicem.py except the first sample script that demonstrates how to get service ticket.
