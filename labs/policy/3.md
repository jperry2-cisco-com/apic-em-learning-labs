...
def select_device_id(ap):
    ...
    device_list = []
    # Extracting attributes,use 'enumerate' to add a counter to an iterable
    ...
    # finf out network device with selected ip or hostname, index 4 is the network device id
    ...
    while True:
        ...
        if user_input.isdigit(): # make sure user's input in in range
            if int(user_input) in range(1,len(device_list)+1):
                net_id = device_list[int(user_input)-1][device_id_idx] # 4 is the position of id
                return net_id
            else:
